...
final class CharsetValidator implements Validator<String> {
	...
	public boolean validate(final Problems problems, final String compName, final String model) {
		...
		try {
			...
			problems.add(ValidationBundle.getMessage(CharsetValidator.class, "ILLEGAL_CHARSET_NAME", compName, model)); // NOI18N
			e = badName;
		} catch (final UnsupportedCharsetException unsup) {
			problems.add(ValidationBundle.getMessage(CharsetValidator.class, "UNSUPPORTED_CHARSET_NAME", compName, model)); // NOI18N
			e = unsup;
		}
		return e == null;
	}
