...
final class HostNameValidator implements Validator<String> {
	...
	public boolean validate(final Problems problems, final String compName, final String model) {
		if (model.length() == 0) {
			problems.add(ValidationBundle.getMessage(HostNameValidator.class, "INVALID_HOST_NAME", compName, model)); // NOI18N
			return false;
		}
		...
		if (parts.length > 4) {
			problems.add(ValidationBundle.getMessage(IpAddressValidator.class, "TOO_MANY_LABELS", model)); // NOI18N
			return false;
		}
		...
		for (int i = 0; i < parts.length; i++) {
			...
			if (i == parts.length - 1 && label.indexOf(":") > 0) {
				...
				if (labelAndPort.length > 2) {
					problems.add(ValidationBundle.getMessage(HostNameValidator.class, "INVALID_PORT", compName, label)); // NOI18N
					return false;
				}
				if (labelAndPort.length == 1) {
					problems.add(ValidationBundle.getMessage(HostNameValidator.class, "INVALID_PORT", compName, "''")); // NOI18N
					return false;
				} else {
					if (label.endsWith(":")) {
						problems.add(ValidationBundle.getMessage(HostNameValidator.class, "TOO_MANY_COLONS", compName, label)); // NOI18N
						return false;
					}
					try {
						final int port = Integer.parseInt(labelAndPort[1]);
						if (port < 0) {
							problems.add(ValidationBundle.getMessage(IpAddressValidator.class, "NEGATIVE_PORT", port)); // NOI18N
							return false;
						} else if (port >= 65536) {
							problems.add(ValidationBundle.getMessage(IpAddressValidator.class, "PORT_TOO_HIGH", port)); // NOI18N
							return false;
						}
					} catch (final NumberFormatException e) {
						problems.add(ValidationBundle.getMessage(HostNameValidator.class, "INVALID_PORT", compName, labelAndPort[1])); // NOI18N
						return false;
					}
