...
final class ValidHostNameOrIPValidator implements Validator<String> {
	...
	public boolean validate(final Problems problems, final String compName, final String model) {
		...
		if (model.indexOf(" ") > 0 || model.indexOf("\t") > 0) {
			problems.add(ValidationBundle.getMessage(ValidHostNameOrIPValidator.class, "HOST_MAY_NOT_CONTAIN_WHITESPACE", compName, model)); // NOI18N
			return false;
		}
		if (parts.length == 0) { // the string "."
			problems.add(ValidationBundle.getMessage(ValidHostNameOrIPValidator.class, "INVALID_HOST_OR_IP", compName, model)); // NOI18N
			return false;
		}
		for (int i = 0; i < parts.length; i++) {
			...
			if (i == parts.length - 1 && s.contains(":")) { // NOI18N
				...
				if (partAndPort.length > 2) {
					problems.add(ValidationBundle.getMessage(ValidHostNameOrIPValidator.class, "TOO_MANY_COLONS", compName, model)); // NOI18N
					return false;
				}
				...
				if (partAndPort.length == 2) {
					try {
						...
						problems.add(ValidationBundle.getMessage(ValidHostNameOrIPValidator.class, "INVALID_PORT", compName, partAndPort[1])); // NOI18N
						return false;
					}
				}
			}
			...
			if (hasIntParts && hasNonIntParts) {
				problems.add(ValidationBundle.getMessage(ValidHostNameOrIPValidator.class, "ADDRESS_CONTAINS_INT_AND_NON_INT_LABELS", compName, model)); // NOI18N
				return false;
			}
		}
